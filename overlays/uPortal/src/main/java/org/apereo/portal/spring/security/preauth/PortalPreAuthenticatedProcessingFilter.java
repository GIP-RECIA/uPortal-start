...
public class PortalPreAuthenticatedProcessingFilter
    ...
    private void doPortalAuthentication(
        ...
        // CAS SLO : get the ticket from the session before invalidating it
        logger.debug("in PortalPreAuthenticatedProcessingFilter - Getting the ticket from session attributes");
        final HttpSession originalSession = request.getSession(false);
        String ticket = null;
        if(originalSession != null){
            if(originalSession.getAttribute("CAS_SERVICE_TICKET") != null){
                ticket = originalSession.getAttribute("CAS_SERVICE_TICKET").toString();
                logger.debug("Got ticket : {}", ticket);
            } else {
                logger.debug("No ticket in session attributes : no mapping for CAS SLO will be created.");
            }
        } else {
            logger.debug("Session is null : no mapping for CAS SLO will be created.");
        }
        ...
        try {
            ...
            // CAS SLO : save the mapping in redis -> new session to ticket
            // The session will be changed one more time so no need to map ticket to session right now
            if(ticket != null){
                logger.debug("CAS SLO : mapping session {} to ticket {}", s.getId(), ticket);
                slocasMappingStorage.saveSessionToST(s.getId(), ticket);
            } else {
                logger.debug("CAS SLO : ticket is null so no mapping will be created");
            }
